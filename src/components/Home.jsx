import React from 'react'
import './home.css'
function Home() {
  return (

    <div className='font-bold w-full h-full bg-black text-white flex flex-col items-center justify-center gap-32 py-16 px-4'>
      <div className='flex flex-col justify-center items-center gap-4 p-8'>
        <div className='head flex-wrap text-5xl lg:text-7xl flex flex-col justify-center items-center'><span>Tailormade to Manage all</span><span>your Data registers</span></div>
        <div className='flex flex-col text-xl justify-center items-center flex-wrap text-gray-400'><span>Operate with your data predictably, securely, and at a lower cost in an intuitive</span><span>and easy to navigate dashboard.Designed to run all your data types.</span></div>
      </div>
      <div className='flex flex-row gap-8'>
        <button className='font-bold bg-white px-4 py-3 text-black rounded-md'>Buy for $24 ^-^</button>
        <button className='font-bold border-2 rounded-md px-4 py-3 border-fuchsia-400 hover:text-zinc-400'>Learn More</button>
      </div>
      <div className='home flex justify-center items-center w-50 skew-z-6'>
        <img className='w-4/5' src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc141a89b7006a587f0ac8_App%20Image-p-2000.jpeg" alt="" />
      </div>
      <div className='flex flex-col gap-8 items-center'>
        <div className='grid grid-cols-5 gap-y-10 gap-x-20 md:gap-x-40 '><img src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc47b5d721c4ea063fdc48_Logo02.svg" alt="" />
        <img src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc47b5d721c4ea063fdc48_Logo02.svg" alt="" />
        <img src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc47b5d721c4ea063fdc48_Logo02.svg" alt="" />
        <img src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc47b5d721c4ea063fdc48_Logo02.svg" alt="" />
        <img src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc47b5d721c4ea063fdc48_Logo02.svg" alt="" />
        <img src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc47b5d721c4ea063fdc48_Logo02.svg" alt="" />
        <img src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc47b5d721c4ea063fdc48_Logo02.svg" alt="" />
        <img src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc47b5d721c4ea063fdc48_Logo02.svg" alt="" />
        <img src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc47b5d721c4ea063fdc48_Logo02.svg" alt="" />
        <img src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc47b5d721c4ea063fdc48_Logo02.svg" alt="" />
        
        </div>
        <h2 className='text-lg text-gray-400'>Trusted by Top-tier product campanies</h2>
      </div>
      <div className='bg-black w-full h-full flex flex-col justify-center items-center p-4 gap-32'>
        <div className='flex flex-col justify-center items-center gap-8 '>
          <span className='text-5xl items-center'>We can help you manage from top to bottom, all the layers.</span>
          <span className='text-gray-400 text-xl'>Lorem ipsum dolor sit amet, consectetur adipiscing elit. At etiam tellus aliquet risus, odio vitae cras viverra elementum.</span>
        </div>
        <div className='w-full flex flex-col lg:flex-row justify-center items-center p-4'>
          <div className='flex flex-col justify-center p-4 gap-8'>
          <span className='text-5xl'>Top Management, to help you see the bigger picture</span>
          <span className='text-gray-400 text-xl'>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Maecenas massa massa id arcu blandit dignissim contum volutpat dolor fermentum, justo tempor.</span>
          <ul className=' flex flex-col gap-2'>
            <li><div className='flex flex-row gap-2 items-center'><img className='w-8 h-8' src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc4a566d06e55f48fbe8a4_Checkmark%20Icon.png" alt="" /><p>Lorem, ipsum dolor.</p></div></li>
            <li><div className='flex flex-row gap-2 items-center'><img className='w-8 h-8' src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc4a566d06e55f48fbe8a4_Checkmark%20Icon.png" alt="" /><p>Lorem, ipsum dolor.</p></div></li>
            <li><div className='flex flex-row gap-2 items-center'><img className='w-8 h-8' src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc4a566d06e55f48fbe8a4_Checkmark%20Icon.png" alt="" /><p>Lorem, ipsum dolor.</p></div></li>
          </ul>
          </div>
          <div className='w-3/5'>
            <img zclassName='w-3/5' src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc4bd6cede115ca195c43f_Feature%20Image%2001.svg" alt="" />
          </div>
        </div>
        <div className='w-full flex flex-col flex-col-reverse lg:flex-row justify-center items-center p-4 gap-8'>
        <div className='w-3/5'>
            <img zclassName='w-3/5' src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc4bd6a0bf2350ffd1ef9f_Feature%20Image%2002.svg" alt="" />
          </div>
          <div className='flex flex-col justify-center p-4 gap-8'>
          <span className='text-5xl'>Top Management, to help you see the bigger picture</span>
          <span className='text-gray-400 text-xl'>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Maecenas massa massa id arcu blandit dignissim contum volutpat dolor fermentum,Lorem, ipsum..</span>
          <ul className='flex flex-col gap-2'>
            <li><div className='flex flex-row gap-2 items-center'><img className='w-8 h-8' src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc4a566d06e55f48fbe8a4_Checkmark%20Icon.png" alt="" /><p>Lorem, ipsum dolor.</p></div></li>
            <li><div className='flex flex-row gap-2 items-center'><img className='w-8 h-8' src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc4a566d06e55f48fbe8a4_Checkmark%20Icon.png" alt="" /><p>Lorem, ipsum dolor.</p></div></li>
            <li><div className='flex flex-row gap-2 items-center'><img className='w-8 h-8' src="https://assets.website-files.com/62bc1260fe7b1f3d37a8ea01/62bc4a566d06e55f48fbe8a4_Checkmark%20Icon.png" alt="" /><p>Lorem, ipsum dolor.</p></div></li>
          </ul>
          </div>
        </div>
        </div>
    </div>
  )
}

export default Home